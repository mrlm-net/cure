...
// GeminiMDCommand generates a GEMINI.md file for Google Gemini CLI auto-discovery.
type GeminiMDCommand struct {
	nonInteractive bool
	force          bool
	dryRun         bool
	outputPath     string
...
func (c *GeminiMDCommand) Name() string        { return "gemini-md" }
func (c *GeminiMDCommand) Description() string { return "Generate GEMINI.md for Google Gemini CLI" }
func (c *GeminiMDCommand) Usage() string {
	return `Usage: cure generate gemini-md [flags]

Generate a GEMINI.md file for Google Gemini CLI auto-discovery.

Interactive mode (default):
  cure generate gemini-md

Non-interactive mode (for CI/CD):
  cure generate gemini-md --non-interactive \
    --name myapp \
...
Flags:
  ...
  --output            Output file path (default: ./GEMINI.md)
  --force             Overwrite existing file without prompting
`
}

func (c *GeminiMDCommand) Flags() *flag.FlagSet {
	fset := flag.NewFlagSet("gemini-md", flag.ContinueOnError)
	...
	fset.StringVar(&c.outputPath, "output", "./GEMINI.md", "Output file path")
...
func (c *GeminiMDCommand) Run(ctx context.Context, tc *terminal.Context) error {
	c.loadDefaults(tc)
...
func (c *GeminiMDCommand) gatherInput(tc *terminal.Context) error {
	if c.nonInteractive {
...
func (c *GeminiMDCommand) validateFlags() error {
	if c.name == "" {
...
func (c *GeminiMDCommand) promptUser(tc *terminal.Context) error {
	prompter := prompt.NewPrompter(tc.Stdout, os.Stdin)
...
func (c *GeminiMDCommand) printSuccess(tc *terminal.Context) {
	...
	fmt.Fprintln(tc.Stdout, "1. Review GEMINI.md and customize sections as needed")
	fmt.Fprintln(tc.Stdout, "2. Commit to version control (git add GEMINI.md && git commit -m \"Add GEMINI.md\")")
	fmt.Fprintln(tc.Stdout, "3. Gemini CLI auto-discovers GEMINI.md in the repository root")
}
