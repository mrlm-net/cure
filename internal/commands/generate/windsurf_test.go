...
func TestWindsurfRulesCommand_NonInteractive(t *testing.T) {
	tmpDir := t.TempDir()
	outputPath := filepath.Join(tmpDir, ".windsurfrules")
...
func TestWindsurfRulesCommand_DryRun(t *testing.T) {
	tmpDir := t.TempDir()
	outputPath := filepath.Join(tmpDir, ".windsurfrules")
...
func BenchmarkWindsurfRulesCommand_DryRun(b *testing.B) {
	var stdout, stderr bytes.Buffer
	tc := &terminal.Context{Stdout: &stdout, Stderr: &stderr, Config: nil}
